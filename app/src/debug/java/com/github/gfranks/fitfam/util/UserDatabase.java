...
public class UserDatabase {
    ...
    public void createUser(FFUser user) {
    ...
    public void updateUser(FFUser user) {
        ContentValues values = new ContentValues();
        values.put(UserSQLiteHelper.COLUMN_ID, user.getId());
        values.put(UserSQLiteHelper.COLUMN_IMAGE, user.getImage());
        values.put(UserSQLiteHelper.COLUMN_FIRST_NAME, user.getFirstName());
    ...
    public FFUser findUserById(String id) {
    ...
    public FFUser findUserByEmail(String email) {
        Cursor cursor = mDatabase.query(UserSQLiteHelper.TABLE_USERS,
    ...
    public void deleteUser(FFUser user) {
        mDatabase.delete(UserSQLiteHelper.TABLE_USERS, UserSQLiteHelper.COLUMN_ID + "=?", new String[]{user.getId()});
    }

    public List<FFUser> getAllUsers() {
        List<FFUser> users = new ArrayList<>();
        ...
        while (!cursor.isAfterLast()) {
            FFUser user = cursorToUser(cursor);
            users.add(user);
            cursor.moveToNext();
        }
    ...
    private FFUser cursorToUser(Cursor cursor) {
        FFUser.Builder builder = new FFUser.Builder();
