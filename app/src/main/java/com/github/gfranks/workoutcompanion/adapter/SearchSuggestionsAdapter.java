...
import com.github.gfranks.workoutcompanion.data.model.WCLocationResult;
...
public class SearchSuggestionsAdapter extends SimpleCursorAdapter {
    ...
    private List<WCLocationResult> mResults;
    ...
    public WCLocationResult getResultItem(int position) {
        return mResults.get(position);
    }

    public void updateWithLocationResults(List<WCLocationResult> results) {
        mResults = results;
        MatrixCursor cursor = new MatrixCursor(COLUMN_NAMES);
        String[] temp = new String[2];
        int id = 0;
        for (WCLocationResult result : mResults) {
